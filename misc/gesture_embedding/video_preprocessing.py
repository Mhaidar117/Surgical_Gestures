    ...
    for file in os.listdir(transcriptions_folder_path):
        ...
        with open(curr_file_path, 'r') as f:
            for line in f:
                ...
                gesture = line[2]
                # takes the whole gesture subsampled by spacing
                curr_blob = [torch.tensor(v) for v in optical_flow_file[start-1: end: spacing]]
                # takes 50 frames subsampled by spacing
                ...
                curr_kinematics_blob = [torch.tensor(v).view(1, 76) for v in kinematics_list[start-1: end: spacing]]
